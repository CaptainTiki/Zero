...
func run() -> void:
	...
	check(player.get("_shells") == 16 and player.get("_weapon") == 2, "Pickup grants shells and equips")
	check(player.add_shells(8), "Shells top up")
	check(player.get("_shells") == 24, "Shell count")
	...
	player._try_shotgun()
	check(player.get("_shells") == 23, "Shot spends one shell")
	...
	player._try_shotgun()
	check(player.get("_shells") == 23, "Cannot fire while pumping")
	player._update_pump(0.7)
	player._try_shotgun()
	check(player.get("_shells") == 22, "Fires again after the pump")

...
func run() -> void:
	...
	var arena = level.get_node("Arena")
	...
	for i in 4:
		await physics_frame
	check(arena.get("_armed") and not arena.get("_started"), "Crossing the line arms the arena but does not seal it yet")
	player.global_position = Vector3(152, 0.4, -90)
	level._process(0.1)
	arena._process(0.1)
	check(arena.get("_started"), "Walking into the plaza seals it")
	...
	while elapsed < 70.0:
		...
		elapsed += 0.25
		await physics_frame
		if arena.get("_beam").visible:
			beam_seen = true
		if arena.get("_wave_index") > waves_seen:
			waves_seen = arena.get("_wave_index")
	...
	# The seal is spawned by the arena set piece, so it is a child of that node.

...
##   level while a Rammer waits below; up on the walks a Rammer comes as fodder;
...
## - the place falls apart, and running out of time ends the run.
...
func run() -> void:
	...
	check(await wait_until(func() -> bool: return count() == 12, 180), "arm 5's pipe sends wave 3 (%d)" % count())
	freeze_enemies()
	...
	check(await wait_until(func() -> bool: return count() == 18, 180), "arm 2's pipe sends wave 4 (%d)" % count())
	freeze_enemies()
	check(count("rammer") == 2, "up on the walk, wave 4's Rammer comes as fodder (%d Rammers)" % count("rammer"))
	var fodder_all_up := func() -> bool:
		for e in get_nodes_in_group("enemies"):
			if e.get_parent() == piece and not String(e.scene_file_path).ends_with("rammer.tscn") and e.global_position.y < 2.0:
				return false
		return true
	...
	check(count() == 18, "regrouping moves enemies, never adds them (%d)" % count())
	...
	check(await wait_until(func() -> bool: return count() == 7, 180), "arm 4's pipe sends wave 5, two Rammers (%d, %d Rammers)" % [count(), count("rammer")])
	...
	await frames(30)
	check(count() == 7, "no wave after the last pipe (%d)" % count())
